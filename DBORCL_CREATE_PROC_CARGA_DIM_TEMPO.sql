...
------------------------------------------------------------------------------------------------
DESCRIÇÃO: PROCEDURE QUE CARREGA TABELA DE DIMENS�O TEMPO PARA FINS DE DATA WHAREHOUSE
           INSERE DATA INFORMADA NO PAR�METRO INICIAL AT� DATA FINAL INCLUSIVE
           CASO EXISTA REGISTROS A DATA_INICIAL � DESCONCIDERADA E UTILIZA-SE A ULTIMA DATA REGISTRADA.
           CASO SEJA INSERIDA UMA DATA_FINAL MENOR QUE A DATA_INICIAL NENHUM REGISTRO � INSERIDO,
           ASSIM COMO A DATA FINAL SEJA MENOR QUE A ULTIMA DATA REGISTRADA
           D�VIDAS, SUGEST�ES: franklinv@gmail.com
------------------------------------------------------------------------------------------------
*/
ULTIMA_DATA   DATE;
DATA_CORRENTE DATE;
...
BEGIN
  ...
  IF ULTIMA_DATA IS NULL THEN
    DATA_CORRENTE := DATA_INICIAL; /*PARA PRIMEIRA CARGA INSERE-SE VALORES PADR�ES PARA DATAS INV�LIDAS E DATAS EM BRANCO (N�O ENCONTRADA)*/
    INSERT INTO TD_TEMPO VALUES(-2,'02/01/0001',-2,-2,'DATA INV�LIDA',-2,'DI',2,'DI',-2,'DI',-2,'DI',-2,'DI',-2,'DI',-2,-2,'DI',-2,'DI',-2);
    INSERT INTO TD_TEMPO VALUES(-1,'01/01/0001',-1,-1,'N�O INFORMADO',-1,'NI',1,'NI',-1,'NI',-1,'NI',-1,'NI',-1,'NI',-1,-1,'NI',-1,'NI',-1);
  ELSE
    DATA_CORRENTE := ULTIMA_DATA + 1;
  END IF; /* CASO ULTIMA_DATA SEJA NULA UTILIZA-SE A DATA_INICIAL SE N�O USA-SE PROXIMA DATA */
    ...
    INSERT INTO TD_TEMPO (CO_SEQ_TEMPO,
                   ...
                   VALUES(TO_NUMBER(TO_CHAR(DATA_CORRENTE,'YYYYMMDD')), -- CO_SEQ_TEMPO
                          ...
                          CASE
                            ...
                            WHEN MES_CORRENTE IN (1,2)   THEN '1� BIMESTRE'
                            WHEN MES_CORRENTE IN (3,4)   THEN '2� BIMESTRE'
                            WHEN MES_CORRENTE IN (5,6)   THEN '3� BIMESTRE'
                            WHEN MES_CORRENTE IN (7,8)   THEN '4� BIMESTRE'
                            WHEN MES_CORRENTE IN (9,10)  THEN '5� BIMESTRE'
                            WHEN MES_CORRENTE IN (11,12) THEN '6� BIMESTRE'
                          END,                                          -- DS_BIMESTRE
                          CASE
                          ...
                          END,                                          -- CO_TRIMESTRE
                          CASE
                            WHEN MES_CORRENTE IN (1,2,3)    THEN '1� TRIMESTRE'
                            WHEN MES_CORRENTE IN (4,5,6)    THEN '2� TRIMESTRE'
                            WHEN MES_CORRENTE IN (7,8,9)    THEN '3� TRIMESTRE'
                            WHEN MES_CORRENTE IN (10,11,12) THEN '4� TRIMESTRE'
                          END,                                          -- DS_TRIMESTRE
                          CASE
                          ...
                          END,                                          -- CO_ESTACAO --(ESTA��ES DO ANO)
                          CASE
                            WHEN (MES_CORRENTE = 12 AND DIA_CORRENTE >= 21) OR MES_CORRENTE IN (1,2)   OR (MES_CORRENTE =  3 AND DIA_CORRENTE < 21) THEN 'VER�O'
                          ...
                          END,                                          --CO_SEMESTRE
                          CASE
                            WHEN MES_CORRENTE IN (1,2,3,4,5,6)    THEN '1� SEMESTRE'
                            WHEN MES_CORRENTE IN (7,8,9,10,11,12) THEN '2� SEMESTRE'
                          END,                                                        --DS_SEMESTRE
